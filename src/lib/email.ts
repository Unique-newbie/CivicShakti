...
export async function sendStatusUpdateEmail(
    ...
    try {
        ...
        const info = await mailer.sendMail({
            from: process.env.SMTP_FROM || '"CivicShakti Support" <noreply@civicshakti.gov>', // sender address
...
export async function sendSubmissionConfirmationEmail(
    ...
    if (!to || !to.includes('@')) {
        console.warn('sendSubmissionConfirmationEmail skipped: No valid email.');
        return null;
    }
    ...
    try {
        ...
        const htmlContent = `
            <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
                <div style="background-color: #7c3aed; color: white; padding: 20px; text-align: center;">
                    <h1 style="margin: 0; font-size: 24px;">CivicShakti</h1>
                    <p style="margin: 5px 0 0 0; opacity: 0.9;">Complaint Submitted Successfully ✅</p>
                </div>
                <div style="padding: 24px; background-color: #ffffff;">
                    <p style="font-size: 16px; color: #374151;">Dear Citizen,</p>
                    <p style="font-size: 16px; color: #374151;">
                    ...
                    <div style="background-color: #f3f4f6; padding: 16px; border-radius: 6px; margin: 20px 0;">
                        <p style="margin: 0 0 10px 0;"><strong>Tracking ID:</strong> <span style="color: #7c3aed; font-size: 18px; font-weight: bold;">${trackingId}</span></p>
                        <p style="margin: 0 0 10px 0;"><strong>Category:</strong> ${category.replace('_', ' ').replace(/\b\w/g, c => c.toUpperCase())}</p>
                        <p style="margin: 0;"><strong>Description:</strong> <em>"${description.substring(0, 200)}${description.length > 200 ? '...' : ''}"</em></p>
                    </div>

                    <p style="font-size: 14px; color: #6b7280;">
                        You can track the progress of your complaint anytime using your Tracking ID on the CivicShakti platform.
                    </p>
                    <p style="font-size: 14px; color: #6b7280;">
                        You will receive email notifications whenever the status of your complaint changes.
                    </p>
                </div>
                <div style="background-color: #f8fafc; padding: 16px; text-align: center; border-top: 1px solid #e5e7eb;">
                    <p style="margin: 0; font-size: 12px; color: #94a3b8;">
                        This is an automated message from CivicShakti. Please do not reply.
                    </p>
                </div>
            </div>
        `;
        ...
        const info = await mailer.sendMail({
            from: process.env.SMTP_FROM || '"CivicShakti" <noreply@civicshakti.gov>',
            to,
            subject: `Complaint Submitted — Tracking ID: ${trackingId}`,
            html: htmlContent,
        });

        console.log("Submission confirmation sent: %s", info.messageId);
        if (!process.env.SMTP_HOST) {
            console.log("Preview URL: %s", nodemailer.getTestMessageUrl(info));
        }
