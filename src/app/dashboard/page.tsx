...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: any }> = {
    ...
    "Under Review": { label: "Under Review", color: "bg-amber-100 text-amber-700 border-amber-200", icon: Eye },
    in_progress: { label: "In Progress", color: "bg-blue-100 text-blue-700 border-blue-200", icon: Activity },
    "In Progress": { label: "In Progress", color: "bg-blue-100 text-blue-700 border-blue-200", icon: Activity },
...
function normalizeStatus(s: string) {
    return s?.toLowerCase().replace(/\s+/g, '_') || 'pending';
}
...
export default function CitizenDashboard() {
        ...
        async function init() {
            try {
                ...
                if (compRes.ok) {
                    ...
                    if (isMounted) setComplaints(compData.documents || []);
                }
            } catch (err) {
                console.error(err);
    ...
    const active = complaints.filter(c => ['pending', 'reviewed', 'in_progress', 'escalated'].includes(normalizeStatus(c.status))).length;
    ...
    const inProgress = complaints.filter(c => normalizeStatus(c.status) === 'in_progress').length;
    const escalated = complaints.filter(c => normalizeStatus(c.status) === 'escalated').length;
    const reviewed = complaints.filter(c => normalizeStatus(c.status) === 'reviewed').length;
                    ...
                    <Card className="rounded-sm border-slate-200 shadow-none">
                        <CardContent className="p-5">
                            ...
                            <div className="flex h-3 rounded-full overflow-hidden bg-slate-100 gap-0.5">
                                {pending > 0 && <div className="bg-slate-400 rounded-full transition-all" style={{ width: `${(pending / total) * 100}%` }} title={`Pending: ${pending}`} />}
                                {reviewed > 0 && <div className="bg-amber-400 rounded-full transition-all" style={{ width: `${(reviewed / total) * 100}%` }} title={`Reviewed: ${reviewed}`} />}
                                {inProgress > 0 && <div className="bg-blue-500 rounded-full transition-all" style={{ width: `${(inProgress / total) * 100}%` }} title={`In Progress: ${inProgress}`} />}
                                {resolved > 0 && <div className="bg-emerald-500 rounded-full transition-all" style={{ width: `${(resolved / total) * 100}%` }} title={`Resolved: ${resolved}`} />}
                                {escalated > 0 && <div className="bg-rose-500 rounded-full transition-all" style={{ width: `${(escalated / total) * 100}%` }} title={`Escalated: ${escalated}`} />}
                            </div>
