...
export default function Home() {
  ...
  const displayStats = [
    {
      value: loading ? "…" : formatStat(stats?.totalComplaints),
      label: t("landing.statComplaints"),
      icon: Zap,
    },
    {
      value: loading ? "…" : formatStat(stats?.resolvedComplaints),
      label: t("landing.statResolved"),
      icon: CheckCircle2,
    },
    {
      value: loading ? "…" : formatStat(stats?.activeAreas),
      label: t("landing.statAreas"),
      icon: MapPin,
    },
    {
      value: loading
        ...
        : stats?.avgResponseHours
          ...
          : "—",
      label: t("landing.statResponseTime"),
      icon: Clock,
    },
  ];
        ...
        <div className="max-w-4xl mx-auto mt-20 grid grid-cols-2 sm:grid-cols-4 gap-6">
          {displayStats.map((stat, i) => (
            <div key={i} className="text-center p-5 rounded-2xl bg-white border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className={`text-2xl sm:text-3xl font-extrabold text-slate-900 ${loading ? "animate-pulse" : ""}`}>
                {stat.value}
              </p>
