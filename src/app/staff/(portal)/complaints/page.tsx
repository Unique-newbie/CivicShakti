    ...
    useEffect(() => {
        async function fetchComplaints() {
            ...
            try {
                ...
                const res = await fetch(`/api/staff/complaints?${params}`, {
                    headers: { Authorization: `Bearer ${jwt}` }
                });
                if (!res.ok) throw new Error('Failed to fetch');
                const data = await res.json();
                
                // Map Appwrite document names format to match the local Complaint interface
                const mappedComplaints: Complaint[] = (data.complaints || []).map((doc: any) => ({
                    id: doc.$id,
                    ticketId: doc.tracking_id,
                    category: doc.category,
                    description: doc.description,
                    address: doc.address || "Location not provided",
                    userId: doc.citizen_contact,
                    imageUrl: doc.image_url,
                    status: doc.status,
                    department: doc.department,
                    createdAt: doc.$createdAt,
                    aiSeverity: doc.ai_severity_score ? String(doc.ai_severity_score) : undefined,
                    state: doc.state_id ? { name: doc.state_id } : undefined,
                    city: doc.city_id ? { name: doc.city_id } : undefined,
                    village: doc.village_id ? { name: doc.village_id } : undefined,
                    ward: doc.ward_id ? { name: doc.ward_id } : undefined,
                }));
        ...
        fetchComplaints();
    }, [currentPage, statusFilter, departmentFilter, categoryFilter]);
    ...
    const handleAutoAssign = async () => {
        ...
        try {
            ...
            const promises = pending.map(c =>
                fetch('/api/staff/complaint_status', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ complaintId: c.id, status: 'In Progress', updatedBy: 'auto-assign' })
                })
            );
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {loading ? (
                                ...
                                paginatedComplaints.map((item) => (
                                    <tr
                                        key={item.id}
                                        onClick={() => router.push(`/staff/complaints/${item.ticketId}`)}
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            {(item.state || item.city || item.village || item.ward) && (
                                                <div className="text-[10px] text-slate-400 mt-0.5 flex items-center gap-1">
                                                    {[item.state?.name, item.city?.name, item.village?.name, item.ward?.name?.replace(/ - .*/, '')].filter(Boolean).join(' → ')}
                                                </div>
                                            )}
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-1 text-slate-600 font-medium">
                                                {/* Upvotes field removed in Prisma migration */}
                                                👍 —
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            {item.aiSeverity !== undefined ? (
                                                <Badge
                                                    variant="secondary"
                                                    className={`
                                                        ${Number(item.aiSeverity) >= 80 ? 'bg-rose-100 text-rose-700 hover:bg-rose-100' : ''}
                                                        ${Number(item.aiSeverity) >= 40 && Number(item.aiSeverity) < 80 ? 'bg-amber-100 text-amber-700 hover:bg-amber-100' : ''}
                                                        ${Number(item.aiSeverity) < 40 ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-100' : ''}
                                                    `}
                                                >
                                                    {item.aiSeverity}/100
                                                </Badge>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <Button variant="ghost" size="icon" className="text-slate-400 hover:text-blue-600" asChild>
                                                <Link href={`/staff/complaints/${item.ticketId}`}>
